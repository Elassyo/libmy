/*
** my_fprintf_nbr_dec.c for libmy in /home/arthur.melin/Code/libmy
**
...
static intmax_t		signed_nbr_to_intmax(int size, va_list *args)
{
  ...
  res = va_arg(*args, intmax_t) & my_fprintf_arg_mask(size);
  sign = res & (1 << (size * 8 - 1));
  if (sign)
    {
      res ^= my_fprintf_arg_mask(size);
      res = -res - 1;
    }
  return (res);
}

int			my_fprintf_nbr_dec_s(int fd, t_fprintf_fmt *fmt,
					     va_list *args, int *written)
{
  ...
  if (nbr < 0 || fmt->flag_plus || fmt->flag_space)
    ...
    nbr_fmt.prefix = "";
  return (my_fprintf_nbr(fd, &nbr_fmt, MY_ABS(nbr), written));
}

int			my_fprintf_nbr_dec_u(int fd, t_fprintf_fmt *fmt,
					     va_list *args, int *written)
{
  uintmax_t		nbr;
  t_fprintf_nbr_fmt	nbr_fmt;
